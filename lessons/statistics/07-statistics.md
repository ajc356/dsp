...
Answer: There is a 29.4% probability that Elvis was an identical twin.  
...
Frequentist and Bayseian statistics are both used to evaluate evidence about competing hypothesis but using notably different approaches that can lead to opposite conclusions about the same data. This is because the two approaches ask fundamentally different questions and are driven by different assumptions. A frequentist approach is concerned with determining whether or not one should reject the null hypothesis whereas a Bayesian is concerned with the probabilities of each hypothesis both null and alternative(s). For a frequentist approach, the only hypothesis that needs to be specified in the null hypothesis  - however the rejection of the null hypothesis does not tell us which of any possible alternatives might be better supported. This frequentist interpretation draws its conclusion on the relative frequency of an event after many trials. By contrast, a Baysenian approach is capable of telling us information about possible alternatives in the form of degree of belief in an event occuring. This, however, depends on the individual Bayesian statistician's "priors" and exactly how confident they are in those priors. That is to say the Bayesian degree of belief is may be based on prior knowledge about the event,  personal beliefs and other subjective factors. Bayesian infererence is by nature subjective -- different analysts will deploy different priors which may lead them to different conclusions. Nevertheless, these priors can be updated and analysis can incorporate new addtional data as it comes in, unlike a frequentist approach in which experiments be fully specified ahead of time. 
